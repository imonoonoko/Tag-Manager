...
class TagManagerApp:
    def __init__(self, root: Any, db_file: Optional[str] = None) -> None:
        ...
        # 絶対パスでデータベースファイルを指定
        from modules.config import DB_FILE # config.pyからDB_FILEをインポート
        self.tag_manager = TagManager(db_file=DB_FILE, parent=self.root)
        # キャッシュを無効化してデータを再読み込み
        self.tag_manager.invalidate_cache()
        self.q: queue.Queue[Any] = queue.Queue()
        self.refresh_debounce_id: Optional[str] = None # ここを追加
        self.search_timer: Optional[str] = None
        self.suggest_listbox: Optional[tk.Listbox] = None
        self.suggest_window: Optional[tk.Toplevel] = None
        self.insert_pos: str = ""
        self.current_suggestions: List[str] = []
        self.selected_suggestion_index: int = 0
        self.output_text: Optional[tk.Text] = None
        self.weight_var: Optional[tk.StringVar] = None
        self.weight_output_text: Optional[tk.Text] = None
        self.weight_selection_text: Optional[tk.Text] = None
        self.progress_dialog: Optional[tk.Toplevel] = None
        self.progress_var: Optional[tk.DoubleVar] = None
        self.progress_label: Optional[tk.Label] = None
        self.progress_cancelled: bool = False
        
        # 編集中のタグ情報を保持
        self._editing_old_tag: Optional[str] = None
        self._editing_is_negative: bool = False
        ...
        self.auto_sort_var: Optional[tk.BooleanVar] = None
        ...
        # 初回起動時のAIモデルダウンロード処理
        self.check_and_download_ai_models()
    ...
    def check_and_download_ai_models(self) -> None:
        """初回起動時のAIモデルダウンロード処理"""
        # --- デバッグ用: 強制的にダウンロードダイアログを表示 --- 
        FORCE_AI_DOWNLOAD_DIALOG = False # デバッグが完了したらFalseに戻す
        if FORCE_AI_DOWNLOAD_DIALOG:
            print("[DEBUG] AIモデルダウンロードダイアログを強制表示します。")
            self.show_ai_model_download_dialog()
            return
        # --- デバッグ用コードここまで ---
        try:
            import os
            ...
            # AI設定ファイルのパスを確認（config.pyから取得）
            from modules.config import AI_SETTINGS_FILE
            ai_settings_path = AI_SETTINGS_FILE
            
            # AI設定を読み込み
            try:
                if not os.path.exists(ai_settings_path):
                    print("AI設定ファイルが見つかりません。初回起動と判断します。")
                    self.show_ai_model_download_dialog()
                    return
                ...
                # 初回起動フラグをチェック
                if ai_settings.get("first_run", True):
                    print("初回起動を検出しました。AIモデルダウンロードダイアログを表示します。")
                    self.show_ai_model_download_dialog()
                    return
                
                # スキップ設定が有効な場合は何もしない
                if ai_settings.get("skip_download", False):
                    print("AIモデルダウンロードがスキップされています。")
                    return
                
                # ダウンロード完了フラグをチェック
                if ai_settings.get("download_completed", False):
                    print("AIモデルダウンロードは既に完了しています。")
                    # 実際のモデルファイルの存在を確認
                    try:
                        ai_predictor = get_ai_predictor()
                        if ai_predictor.is_models_available():
                            print("AIモデルが正常に利用可能です。")
                            return
                        else:
                            print("AIモデルファイルが見つかりません。再ダウンロードが必要です。")
                            # 設定をリセット
                            ai_settings["models_downloaded"] = False
                            ai_settings["download_completed"] = False
                            with open(ai_settings_path, 'w', encoding='utf-8') as f:
                                json.dump(ai_settings, f, indent=2, ensure_ascii=False)
                            self.show_ai_model_download_dialog()
                    except Exception as e:
                        print(f"AIモデル確認エラー: {e}")
                        # エラーが発生した場合はダウンロードダイアログを表示
                        self.show_ai_model_download_dialog()
                else:
                    print("AIモデルがダウンロードされていません。")
                    self.show_ai_model_download_dialog()
            ...
            print(f"AIモデルチェックエラー: {e}")

    def show_ai_model_download_dialog(self) -> None:
        """AIモデルダウンロードダイアログを表示"""
        try:
            # ダウンロードダイアログを作成
            dialog = tk.Toplevel(self.root)
            dialog.title("AIモデルダウンロード")
            ...
            # タイトル
            title_label = tb.Label(main_frame, text="AIモデルの初期化", font=("Arial", 16, "bold"))
            title_label.pack(pady=(0, 20))
            
            # 説明文
            description = """
初回起動時に必要なAIモデルをダウンロードします。
これにより、タグの自動分類機能が利用できるようになります。

ダウンロードには数分かかる場合があります。
インターネット接続が必要です。
            """
            desc_label = tb.Label(main_frame, text=description, justify=tk.LEFT, wraplength=450)
            desc_label.pack(pady=(0, 20))
            
            # プログレスバー
            progress_var = tk.DoubleVar()
            progress_bar = tb.Progressbar(main_frame, variable=progress_var, maximum=100)
            progress_bar.pack(fill=tk.X, pady=(0, 10))
            
            # ステータスラベル
            status_var = tk.StringVar(value="準備中...")
            status_label = tb.Label(main_frame, textvariable=status_var)
            ...
            # ダウンロード開始ボタン
            download_button = tb.Button(button_frame, text="ダウンロード開始", 
                                      command=lambda: start_download(dialog, progress_var, status_var))
            download_button.pack(side=tk.LEFT, padx=(0, 10))
            
            # スキップボタン
            skip_button = tb.Button(button_frame, text="スキップ", 
                                  command=lambda: skip_download(dialog))
            skip_button.pack(side=tk.LEFT)
            
            def start_download(dialog, progress_var, status_var):
                """ダウンロード処理を開始"""
                ...
                def download_worker():
                    try:
                        ...
                        # ステータス更新
                        self.root.after(0, lambda: status_var.set("AI予測器を初期化中..."))
                        self.root.after(0, lambda: progress_var.set(10))
                        
                        # AI予測器を取得
                        ai_predictor = get_ai_predictor()
                        
                        self.root.after(0, lambda: status_var.set("ローカルAIマネージャーを初期化中..."))
                        self.root.after(0, lambda: progress_var.set(20))
                        
                        # ローカルAIマネージャーを初期化
                        hf_manager = LocalHuggingFaceManager()
                        
                        self.root.after(0, lambda: status_var.set("モデルファイルをダウンロード中..."))
                        self.root.after(0, lambda: progress_var.set(30))
                        
                        # モデルをダウンロード
                        hf_manager.download_models()
                        
                        self.root.after(0, lambda: status_var.set("モデルを読み込み中..."))
                        self.root.after(0, lambda: progress_var.set(70))
                        
                        # モデルを読み込み
                        hf_manager.load_models()
                        
                        self.root.after(0, lambda: status_var.set("設定を保存中..."))
                        ...
                        ai_settings = {
                            ...
                            "force_cpu": True,
                            "skip_model_device_assignment": True,
                            "use_cpu_only_initialization": True,
                            "models_downloaded": True,
                            "skip_download": False,
                            "download_completed": True,
                            "first_run": False,  # 初回起動フラグを無効化
                            "last_download_check": datetime.datetime.now().isoformat(),
                        ...
                        self.root.after(0, lambda: status_var.set("完了！"))
                        self.root.after(0, lambda: progress_var.set(100))
                        
                        # 完了メッセージを表示
                        self.root.after(2000, lambda: show_completion_message(dialog))
                        
                    except Exception as e:
                        self.root.after(0, lambda: status_var.set(f"エラー: {str(e)}"))
                        self.root.after(0, lambda: download_button.config(state=tk.NORMAL))
                        self.root.after(0, lambda: skip_button.config(state=tk.NORMAL))
                        print(f"AIモデルダウンロードエラー: {e}")
            ...
            def show_completion_message(dialog):
                """完了メッセージを表示"""
                messagebox.showinfo("完了", "AIモデルのダウンロードが完了しました。\nタグの自動分類機能が利用できるようになりました。", parent=dialog)
                dialog.destroy()
                
        except Exception as e:
            print(f"AIモデルダウンロードダイアログエラー: {e}")
            # エラーが発生した場合はスキップ
            pass
    ...
    def load_categories(self) -> None:
        try:
            # 共通関数を使用してファイルを読み込み
            self.category_keywords = find_and_load_json_file('categories.json') or {}
            
            # ファイルが見つからない場合、デフォルトのカテゴリを設定
            if not self.category_keywords:
                self.logger.warning("categories.jsonが見つかりません。デフォルトのカテゴリを設定します。")
                self.category_keywords = {
                    "品質・画質指定": ["high quality", "best quality", "masterpiece", "lowres", "blurry", "jpeg artifacts", "detailed", "realistic", "sharp", "hdr", "4k", "8k", "ultra high res", "高画質", "低画質"],
                    "スタイル・技法": ["digital art", "watercolor", "pixel art", "anime style", "oil painting", "3d render", "photorealistic", "sketch", "lineart", "manga", "cartoon", "リアル", "アニメ", "イラスト"],
                    "キャラクター設定": ["original character", "oc", "fanart", "male", "female", "boy", "girl", "man", "woman", "child", "adult", "老人", "少年", "少女"],
                    "ポーズ・動作": ["sitting", "standing", "running", "jumping", "pose", "walking", "lying", "crouching", "kneeling", "dance", "jump", "walk", "sit", "stand", "action", "ポーズ", "ジャンプ", "座る", "立つ"],
                    "服装・ファッション": ["dress", "skirt", "jacket", "coat", "hat", "beret", "bow", "ribbon", "gloves", "scarf", "shoes", "boots", "socks", "stockings", "hoodie", "t-shirt", "jeans", "uniform", "kimono", "swimsuit", "浴衣", "制服", "水着", "ドレス", "帽子"],
                    "髪型・髪色": ["long hair", "short hair", "ponytail", "braid", "bangs", "curly", "straight", "wavy", "hairpin", "hairband", "blonde", "black hair", "red hair", "blue hair", "ツインテール", "ポニーテール", "金髪", "黒髪", "青髪", "赤髪"],
                    "表情・感情": ["smile", "smiling", "laugh", "cry", "angry", "sad", "happy", "surprised", "neutral", "serious", "blush", "wink", "frown", "smiling face", "泣く", "笑顔", "怒り", "悲しい", "嬉しい", "真顔", "ウィンク"],
                    "背景・環境": ["background", "city", "forest", "night", "sky", "mountain", "beach", "blur", "bokeh", "sunset", "rain", "snow", "urban", "rural", "海", "山", "森", "空", "夜", "街", "田舎", "夕焼け", "雨", "雪"],
                    "照明・色調": ["bright", "dark", "colorful", "neon", "glowing", "glow", "shadow", "lightning", "sunlight", "vibrant", "pastel", "monochrome", "warm", "cool", "saturated", "desaturated", "muted", "hue", "contrast", "gradient", "soft light", "backlight", "rim light", "ambient light", "diffuse light", "明るい", "暗い", "カラフル", "ネオン", "光", "影"],
                    "小物・アクセサリー": ["bag", "glasses", "necklace", "ring", "earrings", "bracelet", "watch", "belt", "hat", "mask", "umbrella", "phone", "スマホ", "傘", "眼鏡", "指輪", "ネックレス", "イヤリング", "ブレスレット"],
                    "特殊効果・フィルター": ["sparkles", "fog", "vignette", "bokeh", "motion blur", "grain", "noise", "lens flare", "glitch", "fire", "smoke", "sparkle", "炎", "煙", "きらきら", "フィルター", "ノイズ"],
                    "構図・カメラ視点": ["close-up", "wide shot", "bird's eye view", "worm's eye view", "overhead", "side view", "front view", "rear view", "俯瞰", "煽り", "クローズアップ", "引き", "横顔", "後ろ姿"],
                    "ネガティブ": ["bad", "low quality", "worst quality", "error", "blurry", "duplicate", "artifact", "失敗", "低品質", "ノイズ", "エラー", "重複"]
                }
        except Exception as e:
    ...
    def load_prompt_structure_priorities(self) -> None:
        ...
        try:
            # 共通関数を使用してファイルを読み込み
            data = find_and_load_json_file('prompt_structure.json')
            if data:
    ...
    def save_app_settings(self) -> None:
        ...
        try:
            ...
            if hasattr(self, 'theme_manager'):
                self.theme_manager.save_theme_settings(self.theme_manager.current_theme)
            
            # カスタマイズ設定を保存
            if hasattr(self, 'customization_manager'):
                self.customization_manager.user_settings.save_settings()
                self.customization_manager.custom_keywords.save_settings()
                self.customization_manager.custom_rules.save_settings()
            
            # AI学習データを保存
            try:
                from modules.ai_predictor import get_ai_predictor
                ai_predictor = get_ai_predictor()
                if ai_predictor and hasattr(ai_predictor, 'usage_tracker'):
                    ai_predictor.usage_tracker.save_usage_data()
            except Exception as e:
                print(f"AI学習データの保存に失敗: {e}")
                
        except Exception as e:
            print(f"設定の保存に失敗しました: {e}")
    
    def immediate_save(self) -> None:
        """変更が起きる度に即座に保存を実行"""
        try:
            # 設定を保存
            self.save_app_settings()
    ...
    def start_auto_save(self) -> None:
        ...
        try:
            ...
            print(f"自動保存中にエラーが発生しました: {e}")
        
        # 2分後に再度実行（バックアップ用）
        self.auto_save_timer = self.root.after(120000, self.start_auto_save)  # 120000ms = 2分
    ...
    def clear_edit_panel(self) -> None:
        print(f"[DEBUG] clear_edit_panel - 編集パネルをクリア")
        self.entry_tag.delete(0, tk.END)
        self.entry_jp.delete(0, tk.END)
        self.entry_category.delete(0, tk.END)
        # 編集モードをクリア
        self._editing_old_tag = None
        self._editing_is_negative = False
        print(f"[DEBUG] clear_edit_panel - 編集モードをクリア: _editing_old_tag={self._editing_old_tag}, _editing_is_negative={self._editing_is_negative}")

    def save_edit(self) -> None:
        print("[DEBUG] save_edit function entered.")
        tag_new = self.entry_tag.get().strip()
        jp_new = self.entry_jp.get().strip()
        cat_new = self.entry_category.get().strip()
        print(f"[DEBUG] save_edit - Inputs: tag_new={tag_new}, jp_new={jp_new}, cat_new={cat_new}")
        print(f"[DEBUG] save_edit - _editing_old_tag={self._editing_old_tag}, _editing_is_negative={self._editing_is_negative}")
        ...
        # Determine if it's an update or a new tag based on _editing_old_tag
        if self._editing_old_tag: # This means a tag was selected for editing
            old_tag = self._editing_old_tag
            is_negative_for_update = self._editing_is_negative

            # If the new tag name is different from the old one, check for existence
            if old_tag != tag_new and self.tag_manager.exists_tag(tag_new):
                print(f"[DEBUG] save_edit - 重複チェック失敗: {tag_new} は既に存在します")
                messagebox.showerror("エラー", "その英語タグは既に存在します。", parent=self.root)
                return

            # Handle negative tag category constraint
            if is_negative_for_update and cat_new != "ネガティブ":
                messagebox.showerror("エラー", "ネガティブタグのカテゴリは「ネガティブ」に固定されています。", parent=self.root)
                return

            print(f"[DEBUG] save_edit - Updating tag: old_tag={old_tag}, tag_new={tag_new}, jp_new={jp_new}, cat_new={cat_new}, is_negative_for_update={is_negative_for_update}")
            result = self.tag_manager.update_tag(old_tag, tag_new, jp_new, cat_new, is_negative_for_update)
            print(f"[DEBUG] save_edit - update_tag result: {result}")
            if not result:
                messagebox.showerror("エラー", "タグの更新に失敗しました。", parent=self.root)
                return
        ...
        else: # No tag was selected for editing, treat as new addition
            is_negative = (self.current_category == "ネガティブ") # Use current category for new tag's is_negative
            if is_negative and cat_new != "ネガティブ":
                messagebox.showerror("エラー", "ネガティブタグのカテゴリは「ネガティブ」に固定されています。", parent=self.root)
                return
            
            # Check if new tag already exists (for new additions)
            if self.tag_manager.exists_tag(tag_new):
                print(f"[DEBUG] save_edit - 新規追加時の重複チェック失敗: {tag_new} は既に存在します")
                messagebox.showerror("エラー", "その英語タグは既に存在します。", parent=self.root)
                return

            print(f"[DEBUG] save_edit - Adding new tag: tag_new={tag_new}, jp_new={jp_new}, cat_new={cat_new}, is_negative={is_negative}")
            result = self.tag_manager.add_tag(tag_new, is_negative, cat_new)
            print(f"[DEBUG] save_edit - add_tag result: {result}")
            if not result:
        ...
        try:
            ...
            # AI学習エラーは無視（UI操作を継続）
            print(f"AI学習データの記録に失敗: {e}")
            pass
        
        print(f"[DEBUG] save_edit - 保存成功、refresh_tabsを実行")
        self.refresh_tabs()
        # 即座保存を実行
        self.immediate_save()
    ...
    def clear_status_var(self) -> None:
        """ステータス変数をクリアする"""
    ...
    def delete_tag(self) -> None:
        print(f"[DEBUG] delete_tag - 開始")
        tree = self.trees[self.current_category]
        selected = tree.selection()
        print(f"[DEBUG] delete_tag - current_category: {self.current_category}")
        print(f"[DEBUG] delete_tag - selected: {selected}")
        
        if not selected:
            print(f"[DEBUG] delete_tag - 選択されたタグがありません")
        ...
        if self.current_category == "全カテゴリ":
            ...
            for item in selected:
                tag_text = tree.item(item, "values")[0]
                print(f"[DEBUG] delete_tag - 削除対象タグ: {tag_text}")
                if tag_text in tag_info_map:
                    is_negative = tag_info_map[tag_text].get("is_negative", False)
                    print(f"[DEBUG] delete_tag - is_negative: {is_negative}")
                    self.tag_manager.delete_tag(tag_text, is_negative)
        else:
            # 特定のカテゴリの場合は、カテゴリに基づいてis_negativeを判定
            is_negative = (self.current_category == "ネガティブ")
            print(f"[DEBUG] delete_tag - カテゴリベースのis_negative: {is_negative}")
            for item in selected:
                tag_text = tree.item(item, "values")[0]
                print(f"[DEBUG] delete_tag - 削除対象タグ: {tag_text}")
                self.tag_manager.delete_tag(tag_text, is_negative)
        
        print(f"[DEBUG] delete_tag - 削除完了、refresh_tabsを実行")
        self.refresh_tabs()
        self.clear_edit_panel()
        self.clear_weight_selection()
        # 即座保存を実行
        ...
        try:
            for idx, raw_tag in enumerate(tags, 1):
                ...
                for tag in cleaned_tags:
                    category = "ネガティブ" if is_negative else auto_assign_category(tag)
            ...
            # 即座保存を実行
            self.q.put({"type": "immediate_save"})
    ...
    def on_tree_select(self, event: Any) -> None:
        ...
        print(f"[DEBUG] Selected Tag: {tag_text}, JP: {jp_text}, Category: {category_text}")
        
        # 選択されたタグがネガティブタグかどうかを正確に判定
        # is_negativeはDBから取得した値を使用する
        tag_info = self.tag_manager.get_tag_info(tag_text)
        if tag_info:
            is_negative_selected_tag = tag_info.get("is_negative", False)
        else:
            is_negative_selected_tag = (category_text == "ネガティブ") # フォールバック

        # 編集モードの設定
        self._editing_old_tag = tag_text
        self._editing_is_negative = is_negative_selected_tag
        print(f"[DEBUG] on_tree_select - 編集モード設定: _editing_old_tag={self._editing_old_tag}, _editing_is_negative={self._editing_is_negative}")

        # current_categoryを更新して、編集パネルのis_negative判定に影響させる
        if is_negative_selected_tag:
            self.current_category = "ネガティブ"
        else:
            # ポジティブタグの場合は、元のカテゴリに戻すか、全カテゴリにする
            # ここでは、UIの表示カテゴリを維持するために、変更しないか、適切なカテゴリに設定する
            # ただし、save_editのis_negative判定はcurrent_categoryに依存するため、
            # ここでcurrent_categoryをネガティブ以外に設定する必要がある
            if self.current_category == "ネガティブ": # ネガティブカテゴリからポジティブタグを選択した場合
                self.current_category = "全カテゴリ" # またはタグの実際のカテゴリ
        ...
        print(f"[DEBUG] Entry Tag: {self.entry_tag.get()}, Entry JP: {self.entry_jp.get()}, Entry Category: {self.entry_category.get()}")
    ...
    def auto_assign_uncategorized_tags(self) -> None:
        ...
        """
        try:
            # 全タグを取得（ポジティブ・ネガティブ両方）
            all_tags = self.tag_manager.get_all_tags()
            
            # 未分類タグを抽出
            uncategorized_tags = [tag for tag in all_tags if tag.get("category") == "未分類" or tag.get("category") == "" or tag.get("category") is None]
            
            print(f"全タグ数: {len(all_tags)}")
            print(f"未分類タグ数: {len(uncategorized_tags)}")
            ...
            def worker_auto_assign() -> None:
                try:
                    ...
                    all_tag_names = [tag_data.get("tag", "") for tag_data in all_tags]
                    
                    for i, tag_data in enumerate(uncategorized_tags):
                        tag_name = tag_data.get("tag", "")
                        is_negative = tag_data.get("is_negative", False)
                        ...
                        try:
                            ...
                            if confidence < 70.0:
                                ...
                                fallback_category, fallback_details = auto_assign_category_context_aware_pure(
                                    tag_name, category_keywords, CATEGORY_PRIORITIES, all_tag_names
                                )
                            ...
                            if assigned_category != "未分類":
                                if self.tag_manager.set_category(tag_name, assigned_category, is_negative):
                                    assigned_count += 1
                            ...
                            print(f"タグ '{tag_name}' の処理中にエラー: {e}")
                            skipped_count += 1
                            detailed_results.append({
                                "tag": tag_name,
                    ...
                    def show_completion() -> None:
                        ...
                        # 統計情報を作成
                        category_summary = "\n".join([f"・{cat}: {count}個" for cat, count in sorted(category_stats.items(), key=lambda x: x[1], reverse=True)])
                        
                        summary_text = f"""
自動割り当て完了！

■ 処理結果
・処理対象: {len(uncategorized_tags)}個
・割り当て成功: {assigned_count}個
・スキップ: {skipped_count}個
...
詳細な結果を確認しますか？
                        ...
                        result = messagebox.askyesno("自動割り当て完了", summary_text.strip())
                        if result:
                            self.show_detailed_assignment_results(detailed_results, summary_text)
                        else:
                            # タブを更新
                            self.refresh_tabs()
                    ...
                    def show_error() -> None:
                        progress_dialog.close()
                        messagebox.showerror("エラー", "自動割り当て処理中にエラーが発生しました。")
                    
                    # メインスレッドで結果を表示
                    self.root.after(0, show_completion)
                    
                except Exception as e:
                    print(f"自動割り当て処理エラー: {e}")
                    self.root.after(0, show_error)
            
            # ワーカースレッドを開始
            threading.Thread(target=worker_auto_assign, daemon=True).start()
            
        except Exception as e:
            print(f"未分類タグ一括整理エラー: {e}")
            messagebox.showerror("エラー", f"未分類タグの一括整理でエラーが発生しました: {e}")
        ...
        def save_settings() -> None:
            try:
                ...
                if getattr(sys, 'frozen', False):
                    ...
                    settings_file = os.path.join(base_path, 'resources', 'config', 'ai_settings.json')
                else:
                    # 通常のPythonスクリプトから実行されている場合
                    settings_file = os.path.join('resources', 'config', 'ai_settings.json')
...
def find_and_load_json_file(filename: str) -> Optional[Dict[str, Any]]:
    """
    相対パスベースでJSONファイルを検索して読み込む共通関数
    
    Args:
        filename: 読み込むJSONファイル名（例: 'categories.json'）
        
    Returns:
        読み込んだJSONデータ、見つからない場合はNone
    """
    import os
    
    # 相対パスベースの効率的な検索
    base_paths = [
        os.path.dirname(__file__),  # 現在のモジュールディレクトリ
        os.getcwd(),               # カレントワーキングディレクトリ
    ]
    
    # PyInstaller環境の場合はsys._MEIPASSも追加
    if hasattr(sys, '_MEIPASS'):
        base_paths.append(sys._MEIPASS)
    
    # 相対パスの組み合わせ
    relative_paths = [
        f'resources/config/{filename}',
        f'../resources/config/{filename}',
        f'../../resources/config/{filename}',
        filename
    ]
