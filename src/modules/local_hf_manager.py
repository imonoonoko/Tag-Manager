...
try:
    ...
    # transformersのインポートを安全に行う
    try:
        from transformers import AutoTokenizer, AutoModel, pipeline
    except ImportError as e:
        print(f"transformers基本モジュールのインポートエラー: {e}")
        # 部分的なインポートを試行
        try:
            from transformers import AutoTokenizer
            from transformers import AutoModel
        except ImportError:
            print("transformersの部分的なインポートも失敗しました")
    
    # sentence-transformersのインポート
    try:
        from sentence_transformers import SentenceTransformer
        print("Local Hugging Face依存関係の読み込みに成功しました")
        HF_AVAILABLE = True
    except ImportError as e:
        print(f"sentence-transformersのインポートエラー: {e}")
        HF_AVAILABLE = False
    except Exception as e:
        print(f"sentence-transformers初期化エラー: {e}")
        HF_AVAILABLE = False
    ...
    HF_AVAILABLE = False
    logging.warning(f"Local Hugging Face初期化エラー: {e}")
    print(f"Local Hugging Face初期化エラー: {e}")
...
class LocalHuggingFaceManager:
    ...
    def __init__(self, model_name: str = "multilingual", use_gpu: bool = False):
        ...
        try:
            ...
            # モデルが利用可能かチェック
            if not self.is_models_available():
                self.logger.warning("モデルがダウンロードされていません。軽量モードで動作します。")
                self._use_lightweight_embeddings = True
                self._loaded = True
                return
    ...
    def _get_lightweight_embedding(self, tag: str, force_recompute: bool = False) -> Optional[np.ndarray]:
        ...
        try:
            ...
            self.logger.debug(f"軽量埋め込み生成完了: {tag_key}")
            return embedding
    ...
    def cleanup(self):
        ...
        try:
            ...
            logging.error(f"LocalHFManager cleanup error: {e}")
    ...
    def is_models_available(self) -> bool:
        ...
        try:
            ...
            # skip_downloadがTrueの場合は軽量モードで動作
            if ai_settings.get("skip_download", False):
                self.logger.info("skip_downloadがTrueのため、軽量モードで動作します")
                return False
            
            # download_completedがFalseの場合は未ダウンロード
            if not ai_settings.get("download_completed", False):
                self.logger.info("download_completedがFalseのため、モデルが未ダウンロードです")
                return False
            ...
            model_path = os.path.join(MODEL_CACHE_DIR, model_info["name"].replace("/", "_"))
            
            # モデルディレクトリが存在するかチェック
            if not os.path.exists(model_path):
                self.logger.warning(f"モデルディレクトリが存在しません: {model_path}")
                return False
            
            # 必要なファイルが存在するかチェック（より柔軟に）
            required_files = ["config.json"]  # 最低限必要なファイルのみ
            for file_name in required_files:
                file_path = os.path.join(model_path, file_name)
                if not os.path.exists(file_path):
                    self.logger.warning(f"必要なファイルが存在しません: {file_path}")
                    return False
            
            # ファイルが存在する場合は、メタデータファイルの確認をスキップ
            # ダウンロード完了直後はメタデータファイルがまだ更新されていない可能性がある
            self.logger.info(f"モデルファイルが存在します: {model_path}")
            return True
    ...
    def download_models(self) -> bool:
        ...
        try:
            if not HF_AVAILABLE:
                logging.error("Hugging Face Transformersが利用できません")
                return False
            
            model_info = COMMERCIAL_MODELS.get(self.model_name)
            if not model_info:
                logging.error(f"不明なモデル: {self.model_name}")
                return False
            ...
            if os.path.exists(model_path):
                try:
                    ...
                    logging.info(f"既存のモデルキャッシュをクリアしました: {model_path}")
                except Exception as e:
                    logging.warning(f"モデルキャッシュのクリアに失敗しました: {e}")
            
            # モデルをダウンロード
            try:
                from sentence_transformers import SentenceTransformer
                
                logging.info(f"モデルをダウンロード中: {model_name}")
                
                # SentenceTransformerモデルをダウンロード
                embedding_model = SentenceTransformer(model_name, cache_folder=MODEL_CACHE_DIR)
                
                # メタデータを更新
                self._update_model_metadata(model_info, True)

                # ai_settings.jsonのdownload_completedフラグをTrueに設定
                try:
                    from modules.config import AI_SETTINGS_FILE
                    ...
                    ai_settings["download_completed"] = True
                    ai_settings["first_run"] = False
                    ai_settings["last_download_check"] = datetime.now().isoformat()
                    ai_settings["download_date"] = datetime.now().isoformat()
                    ...
                    logging.error(f"ai_settings.jsonの更新に失敗しました: {e}")
                ...
                try:
                    ...
                    if os.path.exists(ai_settings_path):
                        with open(ai_settings_path, 'r', encoding='utf-8') as f:
                            ai_settings = json.load(f)
                    ai_settings["skip_download"] = True
                    ai_settings["download_completed"] = False
                    with open(ai_settings_path, 'w', encoding='utf-8') as f:
                        json.dump(ai_settings, f, indent=2, ensure_ascii=False)
                    logging.info("ダウンロード失敗により軽量モードに切り替えました")
                except Exception as e2:
                    logging.error(f"軽量モード切り替え設定に失敗しました: {e2}")
                return False
    ...
    def _update_model_metadata(self, model_info: dict, downloaded: bool = True):
        """モデルメタデータを更新"""
        try:
            metadata = {}
            if os.path.exists(MODEL_METADATA_FILE):
    ...
    def load_models(self) -> bool:
        ...
        try:
            ...
            # モデルが利用可能かチェック
            if not self.is_models_available():
                logging.error("モデルがダウンロードされていません")
                return False
            ...
            logging.info(f"モデル読み込み完了: {self.model_name}")
            return True
    ...
    def _update_last_used(self):
        ...
        try:
            if os.path.exists(MODEL_METADATA_FILE):
                ...
                if self.model_name in metadata:
                    metadata[self.model_name]["last_used"] = datetime.now().isoformat()
                    
                    with open(MODEL_METADATA_FILE, 'w', encoding='utf-8') as f:
                        json.dump(metadata, f, ensure_ascii=False, indent=2)
                        
        except Exception as e:
            logging.error(f"最終使用日更新エラー: {e}")
