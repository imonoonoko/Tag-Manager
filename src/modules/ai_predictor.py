...
class AIPredictor:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_data_size": len(self.usage_tracker.usage_data) if self.usage_tracker else 0
        }
    ...
    def is_models_available(self) -> bool:
        ...
        try:
            # AI設定ファイルを確認
            import os
            ai_settings_path = os.path.join(os.path.dirname(__file__), '..', 'resources', 'config', 'ai_settings.json')
            ...
            # モデルがダウンロード済みかチェック
            if not ai_settings.get("models_downloaded", False):
                return False
            
            # ローカルAIマネージャーが利用可能かチェック
            try:
                from modules.local_hf_manager import LocalHuggingFaceManager
                hf_manager = LocalHuggingFaceManager()
                return hf_manager.is_models_available()
            except Exception as e:
                print(f"ローカルAIマネージャーチェックエラー: {e}")
                return False
